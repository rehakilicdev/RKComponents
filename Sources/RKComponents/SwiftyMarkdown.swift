...
public enum LineStyle: Int {
    ...
    public static func styleFromString(_ string: String ) -> LineStyle {
        if string == "**" || string == "__" {
            return .bold
...
@objc open class SwiftyMarkdown: NSObject {
    ...
    fileprivate var currentType: LineType = .body
    ...
    func attributedStringFromScanner( _ scanner: Scanner, atStartOfLine start: Bool = false) -> NSAttributedString {
        ...
        if style == .link {
            ...
            if let hasLink = linkText, var hasURL = linkURL as String? {
                followingString = hasLink

                if let startIndex = hasURL.firstIndex(of: "\""),
                    let lastIndex = hasURL.lastIndex(of: "\"") {
                    hasURL.removeSubrange(startIndex...lastIndex)
                }
                hasURL = hasURL.trimmingCharacters(in: .whitespaces)
                attributes[NSAttributedString.Key.link] = hasURL as NSString
            } else {
                style = .none
            }
        } else {
        ...
        guard let attributedString = attributedStringFromString(results.escapedCharacters, withStyle: style).mutableCopy() as? NSMutableAttributedString else { return NSAttributedString() }
        ...
        while matchedCharacters.contains("\\") {
            if let hasRange = matchedCharacters.range(of: "\\") {
                ...
                if matchedCharacters.count > 1 {
                    ...
                    foundCharacters += matchedCharacters[newRange].replacingOccurrences(of: "\\", with: "")
